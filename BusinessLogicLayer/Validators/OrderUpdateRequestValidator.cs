...
internal class OrderUpdateRquestValidator : AbstractValidator<OrderUpdateRequest>
{
    public OrderUpdateRquestValidator()
    {
        RuleFor(x => x.OrderID)
            .NotEmpty().WithErrorCode("Order ID is required.")
            .Must(id => id != Guid.Empty).WithErrorCode("Order ID cannot be an empty GUID.");
